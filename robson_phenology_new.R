...
find_BA_increment_OliverBinks <- function(inDF, type){
  ...
  if (type == "day"){
    ...
    for (tree in trees) {
      ...
      if (!sum(duplicated(thistree$Date))) {
        for (t in 2: dim(thistree)[1]) {
          ...
          inDF$DBH_inc_cm[t] <- (thistree$DBH_cm[t] - thistree$DBH_cm[t-1]) / daysbetween
    ...
    print("month")
    # inDF <- rc_transformed_bymonth # for debugging
    # head(inDF)
    # create an empty column
    inDF$BA_inc_cm2_month <- NA
    inDF$DBH_inc_cm_month <- NA
    inDF$RGR_month <- NA
    # create an empty DF for outputs
    outDF <- inDF[inDF$Tree == "asdasdas", ]
    head(outDF)
    ...
    for (tree in trees) {
      thistree <- inDF[inDF$Tree == tree, ]
      # make sure that the new df is ordered by Date
      thistree <- thistree[order(thistree$year, thistree$month),]
      # only continue when there are no duplicate dates
      if (!sum(duplicated(thistree$month*thistree$year))) {
        for (t in 2: dim(thistree)[1]) {
          # go through the dates in each treeID, and calculate the tree BA increment 
          thisdate     <- as.Date(paste0(thistree$year[t],"-",thistree$month[t],"-", "01"))
          previousdate <-  as.Date(paste0(thistree$year[t-1],"-",thistree$month[t-1],"-", "01"))
          monthsbetween  <- interval(previousdate, thisdate) %/% months(1)
          thistree$DBH_inc_cm_month[t] <- (thistree$DBH_cm[t] - thistree$DBH_cm[t-1]) / monthsbetween
          thistree$BA_inc_cm2_month[t] <- (thistree$BA_cm2[t] - thistree$BA_cm2[t-1]) / monthsbetween
          thistree$RGR_month[t] <- (thistree$DBH_cm[t] - thistree$DBH_cm[t-1]) / thistree$DBH_cm[t-1] / monthsbetween
        }
        # bind the trees into one dataframe
        outDF <- rbind(outDF, thistree)
      }
...
# calculate daily BA, DBH and RGR increments for all the trees
rc_byday <- find_BA_increment_OliverBinks(rc_transformed_byday, type = "day")
head(rc_byday)
# plot to test
plot_test_daily_data(rc_byday)

# calculate monthly BA, DBH and RGR increments for all the trees
rc_bymonth <- find_BA_increment_OliverBinks(rc_transformed_bymonth, type = "month")
rc_bymonth$date <- as.Date(paste0(rc_bymonth$year,"-",rc_bymonth$month,"-","01"))
head(rc_bymonth)

### PLOT FOR TESTING
ggplot(data=rc_bymonth)+
  geom_line(aes(x=date, y=BA_inc_cm2_month))+
  facet_wrap(.~Tree, scales = "free_y")

head(rc_bymonth)
ggplot(data=rc_bymonth)+
  geom_boxplot(aes(x=as.factor(month), y=BA_inc_cm2_month))+
  ylim(-2.5, 2.5)
...
lai_robson <- calc_monthly_lai_incs(lai_robson)

lai_robson <- lai_robson[lai_robson$Date >= "2018-01-01",]
lai_robson <- lai_robson[lai_robson$Date <= "2021-01-01",]

head(lai_robson)

# 
# robson_LAI_inc_byyear <- summaryBy(cbind(lai, lai_inc, lai_inc_month) ~ year+month,
...
# clip the flux data that it will be matching the BAI data (including time time before)
min(rc_bymonth$date)
max(rc_bymonth$date)
flux_data_robson <- flux_data_robson[flux_data_robson$date >= "2018-01-01",]
flux_data_robson <- flux_data_robson[flux_data_robson$date <= "2021-01-01",]
