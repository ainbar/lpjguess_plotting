...
subset_vars_allsites <- rbind(all_withindates[all_withindates$Variable == "BAI_cm2_day",], 
                              ...
                              all_withindates[all_withindates$Variable == "Sws",])
...
SM_ta <- rbind(all_bymonth[all_bymonth$variable == "Sws",],
               all_bymonth[all_bymonth$variable == "Ta",])
## FOR BOX PLOTS
...
Sws_Ta_box <- rbind(all_withindates[all_withindates$Variable == "Sws",],
                   all_withindates[all_withindates$Variable == "Ta",])
...
SM_ta[SM_ta$site == "CUP",]$site <-  "Cumberland Plain"
...
SM_ta[SM_ta$site == "Robson",]$site <-  "Robson Ck"
...
g2 <- ggplot(GPP_NEP, aes(x = month)) +
  ...
  theme(
    ...
    axis.title.y.left = element_text(color = "blue",size=ax_tit_size), # Primary y-axis color
    axis.title.y.right = element_text(color = "red",size=ax_tit_size), # Secondary y-axis color
    axis.text.y.left = element_text(color = "blue",size=ax_tit_size),  # Primary y-axis ticks color
    axis.text.y.right = element_text(color = "red",size=ax_tit_size),   # Secondary y-axis ticks color
    axis.text.x = element_text(size=ax_tit_size)
  )+
...
g1 <- ggplot(ET_Ta, aes(x = month)) +
  ...
  theme(
    legend.position = "none", # Remove legend
    axis.title.y.left = element_text(color = "blue4",size=ax_tit_size), # Primary y-axis color
    axis.title.y.right = element_text(color = "red4",size=ax_tit_size), # Secondary y-axis color
    axis.text.y.left = element_text(color = "blue4",size=ax_tit_size),  # Primary y-axis ticks color
    axis.text.y.right = element_text(color = "red4",size=ax_tit_size),   # Secondary y-axis ticks color
    axis.text.x = element_text(size=ax_tit_size)
  )+
...
g3 <- ggplot(BAI_lai, aes(x = month)) +
  ...
  theme(
    legend.position = "none", # Remove legend
    axis.title.y.left = element_text(color = "green4",size=ax_tit_size), # Primary y-axis color
    axis.title.y.right = element_text(color = "black",size=ax_tit_size), # Secondary y-axis color
    axis.text.y.left = element_text(color = "green4",size=ax_tit_size),  # Primary y-axis ticks color
    axis.text.y.right = element_text(color = "black",size=ax_tit_size),   # Secondary y-axis ticks color
    axis.text.x = element_text(size=ax_tit_size)
  )+
  facet_wrap(~factor(site, levels=c("Robson Ck", "Cumberland Plain", "Wombat")), ncol = 1, scales = "free_y")+
  theme(strip.background = element_blank(), strip.text = element_blank())
...
# ggsave(filename="/Users/30060406/Library/CloudStorage/Dropbox/Presentations/ESA2024/Preso/Figures/obs.jpg", 
#        plot=(g1+g2+g3), width=13, height=6, dpi=300, units="in")
ggsave(filename="/Users/30060406/Library/CloudStorage/Dropbox/Presentations/ESA2024/Preso/Figures/obs_new.jpg", 
       plot=(g1+g2+g3), width=13, height=8, dpi=300, units="in")
########
...
source("~/Library/CloudStorage/Dropbox/R_CODE/HIE/LPJ_allocation_phenology/Penman_PET_ESA2024.R")

SM <- SM_ta[SM_ta$variable == "Ta",]
SM <- rbind(SM)
...
# Create the plot
# Create the plot
maxtemp <- max(SM_ta[SM_ta$variable == "Ta",]$value)
maxET <- max(SM_ta[SM_ta$variable == "Sws",]$value)
scales <- maxET/maxtemp
# scales <- 100/20
head(SM_ta)
g1 <- ggplot(SM_ta, aes(x = month)) +
  geom_line(data = subset(SM_ta, variable == "Sws"), 
            aes(y = value, color = "Sws"), linewidth = 1) +
  geom_line(data = subset(SM_ta, variable == "Ta"), 
            aes(y = value*scales, color = "Ta"), linewidth = 1) +
  geom_point(data = subset(SM_ta, variable == "Sws"), 
             aes(y = value, color = "Sws"), size = 2) +
  geom_point(data = subset(SM_ta, variable == "Ta"), 
             aes(y = value*scales, color = "Ta"), size = 2) +
  # geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
  scale_color_manual(values = c("Sws" = "blue4", "Ta" = "red4"), 
                     name = "Variable") +
  scale_y_continuous(name = "Soil moisture, v/v", 
                     sec.axis = sec_axis(~ . / scales,  name = "mean Temp, ˚c")) +
  # sec.axis = sec_axis(~ . / scale_factor, name = "Var2")
  scale_x_continuous(name = "", breaks = 1:12, labels = month_labels) +
  theme_minimal() +
